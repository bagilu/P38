...
async function loadHandbook() {
  ...
  if (error) {
    status.textContent = "讀取資料失敗：" + error.message;
    return;
  }

  if (!data || data.length === 0) {
    status.textContent = "目前尚無手冊內容。請先執行 SQL 種子資料，或使用 admin.html 新增。";
    tocList.textContent = "尚無章節";
    return;
  }
  ...
  data.forEach((item) => {
    ...
    tocLink.textContent = `${item.display_order}. ${item.section_title}`;
